...
public class BusTrafficSimulator {
    ...
    public void start(Date fromDateTime, Date toDateTime) {
        ...
        Date curTime = fromDateTime;

        reportCollector.sendMessage(this.toString(), "\n****************START SIMULATION****************");

        // The main loop of the simulation
        while (!curTime.after(toDateTime)) {

            reportCollector.sendMessage("", "");
            reportCollector.sendMessage("Clock -> tick", DateTimeHelper.toString(curTime) + ": \n");

            passengerSheduler.tick(curTime);
            long routeId;
            Date timeStart;
            for (RouteDispatcher routeDispatcher : routesDispatcher) {
                routeDispatcher.tick(curTime);
                routeId = routeDispatcher.getRouteForwardId();

                if (timeTable.isExistTask(routeId)) {
                    timeStart = timeTable.peekTask(routeId).getTimeStart();
                    if (DateTimeHelper.diffMinutes(timeStart, curTime) == 0) {
            ...
            bussNetwork.tick(curTime);
            clock.doTick();
            curTime = clock.getCurTime();
        }
        reportCollector.sendMessage(this.toString(), "\n****************FINISH SIMULATION****************");
    }
    ...
    public void loadData(TimeTable timeTable) throws NoSuchFieldException, IllegalAccessException {
...
//        );
        ...
        //routesDispatcher.add(new RouteDispatcher(bussNetwork.getRouteById(1_272), bussNetwork.getRouteById(2_272)));
...
//        );
        //routesDispatcher.add(new RouteDispatcher(bussNetwork.getRouteById(16)));
        ...
        routesDispatcher.add(new RouteDispatcher(bussNetwork.getRouteById(1)));
